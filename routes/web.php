...
// ADMIN LOGIN — named 'login' so the auth middleware redirects here automatically
Route::get('/admin/login',  [AdminLoginController::class,  'showLoginForm'])->name('login')->middleware('guest');
Route::post('/admin/login', [AdminLoginController::class,  'login'])->name('admin.login.post')->middleware('guest');

// AUTHOR LOGIN
Route::get('/author/login',  [AuthorLoginController::class, 'showLoginForm'])->name('author.login')->middleware('guest');
Route::post('/author/login', [AuthorLoginController::class, 'login'])->name('author.login.post')->middleware('guest');

// Redirect the old generic /login so nothing lands on a blank page
...
Route::get('/post-news/trash', [PostNewsController::class, 'trash'])->name('post-news.trash');
Route::post('/post-news/{id}/restore', [PostNewsController::class, 'restore'])->name('post-news.restore');
Route::delete('/post-news/{id}/force-delete', [PostNewsController::class, 'forceDelete'])->name('post-news.force-delete');
Route::get('/post-news/scheduled', [PostNewsController::class, 'scheduled'])->name('post-news.scheduled');
...
Route::middleware(['auth', 'role.admin.writer', 'cors', 'writer.permission'])->group(function () {
 // ADMIN DASHBOARD
...
// AUTHOR PROFILES — must be before the /{slug} catch-all
...
Route::get('/rss', function () {
    $posts = \App\Models\PostNews::with('category')
    ...
    $siteEmail   = \App\Models\WebsiteSetting::getValue('site_email', '');
    $description = \App\Models\WebsiteSetting::getValue('site_default_meta_description', 'Latest news');

    return response()
        ->view('rss.feed', compact('posts', 'siteName', 'siteEmail', 'description'))
        ->header('Content-Type', 'application/rss+xml; charset=utf-8');
})->name('rss');

// PWA MANIFEST — dynamic, pulls site name from WebsiteSetting
Route::get('/manifest.json', [PwaController::class, 'manifest'])->name('pwa.manifest');
Route::get('/offline', fn() => view('offline'))->name('pwa.offline');

// ROBOTS.TXT — served dynamically from WebsiteSetting
Route::get('/robots.txt', function () {
    $content = \App\Models\WebsiteSetting::getValue(
        'robots_txt',
